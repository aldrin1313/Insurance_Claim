...
class Model_Finder:
    ...
    def __init__(self,file_object,logger_object):
        ...
        self.DecisonTreeReg = DecisionTreeRegressor()
    ...
    def get_best_params_for_Decision_Tree_Regressor(self, train_x, train_y):
        ...
        self.logger_object.log(self.file_object,
                               'Entered the DecisionTreeReg method of the Model_Finder class')
        try:
            # initializing with different combination of parameters
            self.param_grid_Decision_Tree = {
                "max_depth": [1, 3, 5, 7, 9, 11, 12],
                "max_features": ["auto", "log2", "sqrt"],
                'criterion': ['squared_error', 'poisson', 'absolute_error', 'None']
                }

            # Creating an object of the Grid Search class
            self.grid = GridSearchCV(self.RandomForestReg, self.param_grid_Decision_Tree, verbose=3, cv=5)
            ...
            self.DecisionTree = DecisionTreeRegressor(max_depth=self.max_depth, max_features=self.max_features, criterion=self.criterion)

            # training the mew models
            self.DecisionTree.fit(train_x, train_y)
            self.logger_object.log(self.file_object,
    ...
    def get_best_model(self,train_x,train_y,test_x,test_y):
        ...
        # create best model for Decision Tree Regression
            self.DecisionTreeReg = self.get_best_params_for_Decision_Tree_Regressor(train_x, train_y)
            self.prediction_DecisionTreeReg = self.DecisionTreeReg.predict(test_x)  # Predictions using the randomForestReg Model
            ...
            if(self.LinearReg_error >  self.prediction_randomForestReg_error):
                if(self.LinearReg_error > self.prediction_DecisionTreeReg_error):
                    return 'LinearRegression', self.LinearReg
                else:
                    return 'DecisionTreeRegressor', self.DecisionTreeReg
            elif(self.prediction_DecisionTreeReg > self.prediction_randomForestReg_error):
                return 'DecisionTreeRegressor', self.DecisionTreeReg
            else:
